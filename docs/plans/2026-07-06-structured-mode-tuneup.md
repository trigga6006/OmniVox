...
>
> **STATUS (2026-07-06, branch `claude/audit-implementation`):** Phase 1 — DONE (config dedupe, n_ctx 4096, cap 4000 chars, `truncated_chars` surfaced in the panel); chunking not built (deliberately deferred). Phase 2 — DONE: prewarm-on-record + non-blocking toggle load, and the `llm-status` event now emits (loading→ready|error) from every load site with a "Loading model" pill label. Phase 4 — DONE differently than written: instead of a file-log toggle, an always-on in-memory ring buffer (last 20 `ExtractionRecord`s in diaglog.rs) feeds `get_llm_diagnostics` + a Recent-extractions disclosure on the Models→LLM card; env-var file log stays for devs; stale-doc banner added. Phase 3 (personas: agent-prompt + email + notes-outline per user decision) — IN PROGRESS on a worktree agent. Phase 5 — open (evidence-gated).

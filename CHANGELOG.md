...
- **Dictations no longer vanish on every other press when Alt is the first key of your hotkey.** Only the key that completes the hotkey chord is intercepted; the first key's press and release still reach the app you're typing into, so real shortcuts like Alt+Tab keep working. But when that first key is Alt (or Win), the app receives what looks like a bare Alt tap — and in Chromium and Electron apps such as Claude, Slack, or VS Code a bare Alt tap toggles keyboard focus onto the menu bar. The window stays in front, every safety check passes, and the pasted text lands in the menu instead of your text field, on exactly alternate dictations. OmniVox now sends one harmless unassigned keystroke while the modifier is still held, so the app never sees a bare Alt tap. Every hotkey press also records which key completed the chord in `model-load.log`, so a report of this shape is diagnosable from the log alone. Pressing an unrelated key while holding the hotkey chord no longer starts or stops a dictation, either.
- **The floating pill can no longer steal focus from the window you're dictating into, and a refused paste now tells you.** Clicking the pill could make it the foreground window; OmniVox refuses to paste unless the window that was in front when you pressed the hotkey still is, so those dictations were silently dropped. The pill is now non-activatable — it becomes focusable only while its draft editor or a Structured Mode panel is open — and when a paste is refused the pill shows *Output failed: …* instead of a toast in the (usually hidden) main window. Before pasting, OmniVox also waits up to 400 ms for a still-held Alt, Win, or Shift to clear, and every output attempt writes a one-line breadcrumb (route, target, result) to `model-load.log`.

### Security

- Updated `rustls` to 0.23.45 to pick up the fix for RUSTSEC-2026-0285 (TLS 1.3 handshake messages accepted across encryption-level boundaries). OmniVox only uses TLS for the optional OpenRouter meeting-notes feature and model downloads; no user action is needed.
...
- **Verified releases.** Every release now publishes a `SHA256SUMS.txt` manifest, and `install.ps1` refuses to execute anything it hasn't verified: it pins the exact expected asset names, rejects any download URL that isn't a github.com release download, checks the downloaded size against the release metadata, verifies the SHA-256 against the published manifest, and cleans up its temp directory on every path. Releases are not code-signed today (expect Windows SmartScreen's unsigned-publisher prompt) — but the pipeline signs and Authenticode-verifies automatically the moment a certificate is configured, and the installer refuses any binary whose signature is present but invalid.
